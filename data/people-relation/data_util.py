...
with codecs.open('train.txt','r','utf-8') as tfc: 
    for lines in tfc:
        ...
        if count[relation2id[line[2]]] <1500:
            ...
            line[3] = str(' '.join(jieba.cut(line[3][:index11])))+' '+line[3][index11:index11+len1]+' '+\
                      str(' '.join(jieba.cut(line[3][index11+len1:index22])))+' '+line[3][index22:index22+len2]+' '+\
                      str(' '.join(jieba.cut(line[3][index22+len2:])))
            line[3] = line[3].split()
...
df_data = pd.DataFrame({'words': datas, 'tags': labels,'positionE1':positionE1,'positionE2':positionE2}, index=list(range(len(datas))))
...
import pickle
with open('../people_relation_train3.pkl', 'wb') as outp:
...
with codecs.open('train.txt','r','utf-8') as tfc:
    for lines in tfc:
        ...
        if count[relation2id[line[2]]] >1500 and count[relation2id[line[2]]]<=1800:
            ...
            line[3] = str(' '.join(jieba.cut(line[3][:index11]))) + ' ' + line[3][index11:index11 + len1] + ' ' + \
                      str(' '.join(jieba.cut(line[3][index11 + len1:index22]))) + ' ' + line[3][
                                                                                        index22:index22 + len2] + ' ' + \
                      str(' '.join(jieba.cut(line[3][index22 + len2:])))
            line[3] = line[3].split()
...
import pickle
with open('../people_relation_test3.pkl', 'wb') as outp:
	pickle.dump(datas, outp)
	pickle.dump(labels, outp)
	pickle.dump(positionE1, outp)
	pickle.dump(positionE2, outp)
